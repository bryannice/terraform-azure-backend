...
.PHONY: tf-destroy
tf-destroy: change-to-local tf-init tf-plan
	@echo "$(BOLD)$(YELLOW)Destroying backend infrastructure in Azure.$(RESET)"
	@sleep 10
	@terraform destroy \
		-auto-approve \
		-input=false \
...
.PHONY: tf-apply
tf-apply: change-to-local tf-init tf-plan
	@echo "$(BOLD)$(YELLOW)Creating backend infrastructure in Azure.$(RESET)"
	@sleep 10
	@terraform apply \
		-input=false \
    	-auto-approve
...
.PHONY: tf-backend
tf-backend: tf-apply change-to-azurerm
	@echo "$(BOLD)$(YELLOW)Initializing terraform to sync statefile with Azure storage account.$(RESET)"
	@sleep 10
	@terraform init \
		-force-copy \
		-input=false
	@echo "$(BOLD)$(GREEN)Completed syncing statefile with Azure storage account.$(RESET)"